...
FROM
    public.category c
    INNER JOIN public.film_category fc ON fc.category_id = c.category_id
GROUP BY
    c.name;
...
WITH
    films_with_no_rentals AS (
        ...
        FROM
            public.rental r
            FULL OUTER JOIN public.inventory i ON r.inventory_id = i.inventory_id
        WHERE
...
FROM
    public.film f
    INNER JOIN films_with_no_rentals fwnr ON f.film_id = fwnr.film_id
